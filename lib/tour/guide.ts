...
const PRINT: GuideAnswer = {
  ...
  a: "Approved printed orders wait in Print, oldest first. Place the order with the print company on their own site, then press Mark as sent here. When the tracking number arrives, press Add tracking on the same card and save it; the order shows as shipped.",
};

const ROSTER: GuideAnswer = {
  q: "How are new orders shared out?",
  a: "New orders go down the Designers list from the top, only to designers who do that portrait style, and never past a designer's daily limit. Change the order, styles or limits on the Designers page. The same page adds designers, VAs and admins, resets passwords, and deactivates anyone who leaves (they drop off the list and their history stays).",
};
